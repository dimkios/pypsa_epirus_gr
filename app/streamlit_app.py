...
with st.container(key="scenario_box"):
    st.header("Σενάριο")
    st.caption(
    ...
    with col_res:
        new_res_mw = st.slider(
            ...
            step=10,
            help=(
                "ΕΠΙΠΛΕΟΝ ισχύς πάνω από την ήδη υπαρκτή (~522MW, βλ. tab 'Generator'). "
    ...
    with col_restype:
        new_res_type = st.selectbox(
            ...
            help=(
                "Επηρεάζει μόνο την ετικέτα/carrier της γεννήτριας. Η διαθεσιμότητά της "
                "(`p_max_pu`) ακολουθεί πάντα το ίδιο πραγματικό ηλιακό προφίλ PVGIS (Ιωάννινα, "
                "2020) — δεν έχουμε ακόμα πραγματικό προφίλ ανέμου, οπότε αν επιλέξεις 'Αιολικά' "
                "θεωρείται απλοποιητικά ότι έχει την ίδια διαθεσιμότητα με τα φωτοβολταϊκά."
            ),
        )
    with col_import:
        import_limit_mw = st.slider(
            ...
            step=50,
            help=(
                "Προεπιλογή 500MW = η πραγματική χωρητικότητα της διασύνδεσης GRITA "
                "Ελλάδας-Ιταλίας (Galatina-Άραχθος, 2001), που καταλήγει στο ΚΥΤ Άραχθος. Πάνω "
                "από 500MW εξομοιώνει υποθετική μελλοντική ενίσχυση (π.χ. GRITA 2, 1000MW, "
                "καταλήγει όμως στη Θεσπρωτία στην πραγματικότητα - δεν είναι ακόμα στο μοντέλο "
                "μας). 0 = πλήρης ενεργειακή ανεξαρτησία Ηπείρου."
            ),
        )
    with col_demand:
    ...
    with col_batt:
        battery_mw = st.slider(
            ...
            help=(
                ...
                "δοκίμασε να προσθέσεις πολλή ΑΠΕ ΚΑΙ να μειώσεις τη ζήτηση για να τη δεις να "
                "δουλεύει."
            ),
        )
